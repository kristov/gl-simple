...
struct gl_simple_stereo ostereo;
...
void draw_scene(struct gl_simple_stereo* ostereo, void* data) {
    gl_simple_draw_rcs(&render, &matrix);
}

GLvoid display(GLvoid) {
    //glClear(GL_COLOR_BUFFER_BIT|GL_DEPTH_BUFFER_BIT);
    gl_simple_stereo_display(&ostereo);
    glutSwapBuffers();
}
...
void init_gl_simple(uint16_t width, uint16_t height) {
    ...
    float physical_width = 1.347;
    gl_simple_stereo_init(&ostereo, width, height, physical_width, &err);
    gl_simple_stereo_draw_scene_callback(&ostereo, draw_scene, NULL);
